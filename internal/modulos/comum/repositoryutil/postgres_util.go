...
func CarregarPerfilPublicoAutorOpcional(ctx context.Context, pool *pgxpool.Pool, usuarioID string, destino *comum.PerfilPublicoAutor) {
	if usuarioID == "" {
		return
	}
	if err := CarregarPerfilPublicoAutor(ctx, pool, usuarioID, destino); err != nil {
		destino.Identificador = usuarioID
	}
}
